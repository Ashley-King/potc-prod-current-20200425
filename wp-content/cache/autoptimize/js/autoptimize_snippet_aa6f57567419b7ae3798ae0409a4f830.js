...
return false;}
$('.form-submit').click(function(e){e.preventDefault();formData=$('#1xq8nm').serialize();if(!isValidEmail()){alert('add email');}else if(!isValidName()){alert('add name')}else if(!notBotz()){alert("no botz please")}else{formSubmit()}});function formSubmit(){$.ajax({type:'POST',url:'/wp-content/themes/potc/ml-submit.php',data:$('#1xq8nm').serialize(),success:function(data){console.log(data)}});}});