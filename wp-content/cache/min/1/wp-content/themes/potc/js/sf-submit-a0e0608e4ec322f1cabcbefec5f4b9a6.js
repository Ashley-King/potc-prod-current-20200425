...
$('.form-submit').click(function(e){e.preventDefault();formData=$('#1xq8nm').serialize();if(!isValidEmail()){alert('add email')}else if(!isValidName()){alert('add name')}else if(!notBotz()){alert("no botz please")}else{$('#1xq8nm input[name="email"]').val('');$('#1xq8nm input[name="first_name"]').val('');$('#1xq8nm input[name="a_password"]').val('');formSubmit()}});function formSubmit(){$.ajax({type:'POST',url:'/wp-content/themes/potc/sf-submit.php',data:formData,success:function(data){window.location.href='https://pediatricotcourses.com/thank-you'}})}})