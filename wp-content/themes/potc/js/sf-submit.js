jQuery(document).ready(function($){
    ...
    function isValidName(){
        $name = $('#1xq8nm input[name="first_name"]').val();
        if($name.length && $name !== ''){
            return true;
        }
        return false;
    ...
    function notBotz(){
        $botz = $('#1xq8nm input[name="a_password"]').val();
        if(!$botz.length || $botz === '' || $botz== undefined || !$botz ){
            return true;
        }
        $('#1xq8nm input[name="email"]').val('');
        $('#1xq8nm input[name="first_name"]').val('');
        $('#1xq8nm input[name="a_password"]').val('');
        return false;
    }
    ...
    function formSubmit(){
        $.ajax({
            type: 'POST',
            url: '/wp-content/themes/potc/sf-submit.php',
            data: formData,
