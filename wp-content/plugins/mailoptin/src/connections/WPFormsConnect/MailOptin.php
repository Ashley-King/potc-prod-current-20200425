...
class MailOptin extends \WPForms_Provider
{
    ...
    public function output_options($connection_id = '', $connection = array())
    {
        ...
        if (defined('MAILOPTIN_DETACH_LIBSODIUM')) {
            ...
            $output        .= '<p>' . sprintf(esc_html__('Upgrade to %s to remove the 500 subscribers per month limit, add support for custom fields mapping and assign tags to subscribers.', 'mailoptin'), '<strong>MailOptin premium</strong>') . '</p>';
